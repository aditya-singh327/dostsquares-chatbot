...
# ✅ OpenRouter Config (API key from .env)
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY")   # ✅ now from .env
...
def handle_llm(state: ChatState) -> ChatState:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"answer": f"❌ LLM Error: {response.text}", "next": "end"}
    except Exception as e:
        return {"answer": f"❌ Server Error: {str(e)}", "next": "end"}
